import { Controller, Get, Post, Body, Put, Param, Delete, Next, InternalServerErrorException, Res, HttpStatus, UsePipes, BadRequestException, NotFoundException, HttpException } from '@nestjs/common';
import { ApiCreatedResponse, ApiNoContentResponse, ApiOkResponse, ApiTags } from '@nestjs/swagger';
import { NextFunction, Response } from 'express';
import { ApiPaginatedResponse } from 'src/decorators/paginator';
import { AllExceptionsFilter } from 'src/filters/all-exceptions';
...
export class CoursesController {
  ...
  create(@Res() res: Response, @Body() createCourseDto: CreateCourseDto, @Next() next: NextFunction): void {
    this.coursesService.create(createCourseDto)
      .then(course => { res.status(HttpStatus.CREATED).send(course) })
  ...
  @Put(':id')
  @ApiOkResponse({ type: CreateCourseDto })
  update(@Param('id') id: string, @Res() res: Response, @Body(new JoiValidationPipe(createCourseSchema)) updateCourseDto: UpdateCourseDto, @Next() next: NextFunction): void {
